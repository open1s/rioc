    ...
    let struct_export_outputs = struct_exports
        ...
        .map(|t| {
            ...
            quote!{
                ...
                impl<'prov, #(#generic_params,)*NjectProvider> rioc::RefInjectable<'prov, #ty, NjectProvider> for #ident<#(#generic_keys),*>
                    where
                        #prov_lifetimes
                        NjectProvider: #(rioc::Provider<'prov, #prov_types>)+*, #where_predicates
                {
                    #[inline]
    ...
    let export_outputs = export_attr_indexes.iter().map(|(i, attrs)| {
        ...
        let outputs = inputs.map(|input| {
            ...
            quote! {
                ...
                impl<'prov, #(#generic_params,)*NjectProvider> rioc::Injectable<'prov, #ty, NjectProvider> for #ty
                    where
                        #prov_lifetimes
                        NjectProvider: rioc::Import<#ident<#(#generic_keys),*>>, #where_predicates
                {
                    #[inline]
